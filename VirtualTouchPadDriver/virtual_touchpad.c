...
bool useTwoFingerTouchScreen = true;
...
static ssize_t show_two_finger_touchpad(struct device *dev,struct device_attribute *attr,char *buf){
    return snprintf(buf,PAGE_SIZE,"%d\n",useTwoFingerTouchScreen);
}
static ssize_t store_two_finger_touchpad(struct device *dev,struct device_attribute *attr,const char *buf,size_t count){
    int tmp;
    sscanf(buf,"%d",&tmp);
    if(tmp!=1&&tmp!=0){
	tmp=0;
    }
    useTwoFingerTouchScreen=tmp;
    return PAGE_SIZE;
}


static DEVICE_ATTR(two_finger_as_touchpad,S_IWUSR|S_IRUGO,show_two_finger_touchpad,store_two_finger_touchpad);
...
static int __init vtp_init(void) {
    ...
    device_create_file(vtp_device,&dev_attr_two_finger_as_touchpad);
